## #1 onlyOneRightAnswer works by running a while loop to retake input until the input answer matches the correct answer
## #2 notAFactorial will run only once as its currently written
## #3 The conditional for the while loop should test whether num is greater than 0. Also, total should equal num * num instead of total * num.
## #4 To make the loop run forever, set the conditional to just be True
## #5 while(num==num) will always evaluate to true, therefore it will never terminate the loop.
...
def magic8(num):
    ...
    else: return "concentrate and ask again"
