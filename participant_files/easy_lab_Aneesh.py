...
#1 Until the user inputs the correct question the while loop keeps looping
# and print the message "Hmm..., lets try that again", once the while loop
# is true the funtion or the code exits

#2 10 times 

#3 the while condition is wrong

#4 In while loop add another or condition where we check for even "y" condition
# If the user input either "y" or "n", make the while loop true

#5 O(n) run time, as long as n > 0
...
def magic8(num):
    ...
    resultStr = ""
    if num < 23:
        resultStr = "ask again later"
    elif num == 41:
        resultStr = "all signs point to yes"
    elif num <= 72:
        resultStr = "outlook not so good"
    else:
        resultStr = "concentrate and ask again"
